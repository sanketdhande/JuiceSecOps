...
# The only LLM provider in this project: every call below goes through the
# Hugging Face `transformers` text-generation pipeline against
# `openai/gpt-oss-20b` (https://huggingface.co/openai/gpt-oss-20b), exactly
# as shown on the model card:
#
...
#   )
#
# Select it with --provider huggingface (see cli.py's _provider()), or omit
# --provider entirely since it's the only one and the default.
import json
import re
import time
...
class HuggingFaceSecurityProvider:
    """LLM review/triage via the `openai/gpt-oss-20b` transformers pipeline."""
