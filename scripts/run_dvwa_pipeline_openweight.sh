#!/usr/bin/env bash
# Same flow as run_dvwa_pipeline_hf.sh, but --provider openweight: the
# diff-review and triage stages in pipeline.py call a small curated
...
# '.[hf,dev]'` and enough local GPU/CPU memory to host an ~8B model -- this
# is still local-only and never runs in GitHub Actions.
#
# Usage: ./scripts/run_dvwa_pipeline_openweight.sh [target-repo] [model-alias-or-id]
# model-alias-or-id defaults to foundation-sec-8b-reasoning. See
# providers/openweight.py MODEL_CHOICES for the other aliases
# (foundation-sec-8b, pentest-7b, qwen-coder-7b, codegemma-7b).
set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
TARGET_REPO="${1:-${ROOT_DIR}/targets/dvwa}"
MODEL_ID="${2:-foundation-sec-8b-reasoning}"
OUTPUT_DIR="${ROOT_DIR}/results/dvwa-openweight"
